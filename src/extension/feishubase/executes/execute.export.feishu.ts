const handleExportFeishu = (datas, feishuTableTabId, matchs) => {
  ...
  new ExecuteHandle()
    ...
    .waitPipe(async (ctx) => {
      // step 5: 拿到授权码
      const copyBtn = await ctx.getDomByQuery(
        'div[class="open-extension-container__personal-token__content-token-input"]'
      );
    ...
    .waitPipe(async (ctx, args) => {
      ...
      const copyBtn = await ctx.getDomByQuery(
        'div[data-focus-lock-disabled="false"] .ud__modal__footer__btns button',
        '确定'
      );
      copyBtn && copyBtn.click();
    })
