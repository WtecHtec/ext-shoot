        ...
        Object.keys(snapsData).map((key) => {
          const datas = snapsData[key]
            ...
            .map((item) => {
              ...
              item.searchTags = [];
              while (fidx > -1) {
                item.searchTags.push([
                  text.substring(fidx - diff, fidx),
                  value,
                  text.substring(fidx + value.length, fidx + diff)
                ]);
    ...
    <div snap-seek-cmdk-list="">
      {Object.keys(displayData).length <= 0 ? (
        ...
        Object.keys(displayData).map((item) => (
          <>
            {displayData[item].length === 0 ? null : (
