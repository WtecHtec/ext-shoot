...
import { ApiOptions, JikeClient } from 'jike-api';
...
export const getPostDetails = async ({ postId, postType }: { postId: string; postType: 'originalPost' | 'repost' }) => {
  const client = await initJikeClient();
...
export const deletePostById = async ({
  ...
  // 确定帖子类型
  const type_ =
    postType === 'originalPost' || postType === 'ORIGINAL_POST'
...
export const cretatImagesPost = async (content: string, ...imageURLs: string[]) => {
  ...
  const result = await client.apiClient.posts.create(ApiOptions.PostType.ORIGINAL, content, {
    pictureKeys: imageKeys.map((image) => image.key)
  });
  return result;
};
